# Derived from COMET-FEniCSx, (c) Jeremy Bleyer, licensed CC BY-SA 4.0
# (https://github.com/bleyerj/comet-fenicsx, tour "Cohesive zone modeling restricted
# to an interface"). This file is redistributed under the same CC BY-SA 4.0 license.
# Modifications for diffcohesive's cross-code benchmark: pyvista/IPython display code removed
# so the script runs without a GUI; the reaction-force curve is written to
# fenicsx_reference_curve.csv.
# ---
# jupyter:
#   jupytext:
...
# --- write the force-displacement curve for the cross-code comparison ---
import numpy as _np
_np.savetxt("fenicsx_reference_curve.csv",
            _np.column_stack([loading, Force]), delimiter=",",
            header="delta,F", comments="")
print("peak force:", float(_np.max(Force)))
print("wrote fenicsx_reference_curve.csv with", len(Force), "points")
